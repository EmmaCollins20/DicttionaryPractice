#Emma Collins        10/21/23
# COM S 127 Section F  
#Description: Player 0, Player 1, or Player 2 play against each other and if on the grid they input 4 charchters in a row they win
...
def printTitleMaterial():
    ...
    print("By: Emma Collins")
    print("[COM S 127 section f")
    print()
...
def checkAdjacent(board, playerNumber):
    ...
    for column in range(0, len(board[0])):
        ...
        if row != -1:
            ...
            if row - 1 >= 0 and row-1 <(len(board)) and column - 1 >= 0 and column-1<(len(board)):
            ...
            if row >= 0 and row <(len(board)) and  column -1 >= 0 and column-1<(len(board)): 
                # do a check to see if the board location under consideration is equal to the piece found above
            ...
            # 
            # ensure we don't get an 'index out of range' error
            if row +1 >= 0  and row +1 <(len(board)) and  column >= 0 and column<(len(board)):
                # do a check to see if the board location under consideration is equal to the piece found above
